...
public class ArrayTests(ITestOutputHelper testOutput) : ParserTestBase(testOutput) {
	...
	public void AssigningNumericIndexSetsLength() {
		...
		a.Get(new Number(8)).ShouldBe(Null.Instance);
	}
	...
	public void MissingElementsAreAlwaysMysterious() {
		var a = new Array();
		a.Get(new Number(2)).ShouldBe(Mysterious.Instance);
		a.Get(new Strïng("foo")).ShouldBe(Mysterious.Instance);
		a.Get(Booleän.False).ShouldBe(Mysterious.Instance);
		a.Get(Booleän.True).ShouldBe(Mysterious.Instance);
		a.Get(a).ShouldBe(Mysterious.Instance);
		a.Get(new Null()).ShouldBe(Mysterious.Instance);
		a.Get(Mysterious.Instance).ShouldBe(Mysterious.Instance);
	}
}